...
Entry point: ``python -m social_home.app`` (or via ``social_home/__main__.py``).
"""
...
def _build_repos(db: AsyncDatabase):
    ...
    the only place that knows about :mod:`social_home.repositories` —
...
def create_app(config: Config | None = None) -> web.Application:
    ...
    # social_home.* events from the HA side.
    ...
    async def _on_startup(app: web.Application) -> None:  # noqa: RUF029
        ...
        log.info("social-home: starting up (mode=%s)", config.mode)
        await db.startup()
    ...
    async def _on_cleanup(app: web.Application) -> None:  # noqa: RUF029
        log.info("social-home: shutting down")
